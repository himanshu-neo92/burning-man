...
namespace octet {
    namespace scene {
        ...
        public:
            ...
            unsigned add_attribute(unsigned attr, unsigned size, unsigned kind, unsigned offset, unsigned norm = 0, unsigned divisor = 0) {
                assert(num_slots < max_slots);
                format[num_slots] = ((divisor > 0 ? 1 : 0) << 15) + (offset << 9) + (attr << 5) + ((size - 1) << 3) + (kind - GL_BYTE);
            ...
            unsigned get_divisor(unsigned slot) const{
                return ((format[slot] >> 15) & 0x01);
            }
            ...
            void set_num_instances(int inst)
            {
                instances = inst;
            }
